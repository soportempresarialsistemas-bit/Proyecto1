...
const BEN_TYPES = ["Personas", "Negocios", "Entidades financieras", "Entidades de Gobierno", "Cuenta propia", "Empresas de Servicios / suscripciones"];
...
function BenForm({ initial, onSubmit, onClose }: { initial?: any; onSubmit: (d: any) => void; onClose: () => void }) {
  ...
  const isPersona = form.type === "Personas";
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2 space-y-2">
          ...
          <Select value={form.type} onValueChange={v => set("type", v)}>
            ...
            <SelectContent>{BEN_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div className="space-y-2"><Label>{isPersona ? "Nombres *" : "Nombre / Razón Social *"}</Label><Input value={form.firstName} onChange={e => set("firstName", e.target.value)} /></div>
        <div className="space-y-2">
          <Label>Apellidos</Label>
          <Input value={form.lastName} onChange={e => set("lastName", e.target.value)} disabled={!isPersona} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
...
export default function ConfigBeneficiaries() {
  const qc = useQueryClient();
  const { toast } = useToast();
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<any>(null);
  const list = useQuery({ queryKey: ["beneficiaries"], queryFn: () => localApi<any[]>("/beneficiaries") });
  const create = useMutation({
    mutationFn: (d: any) => localApi("/beneficiaries", { method: "POST", body: JSON.stringify(d) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["beneficiaries"] }); setOpen(false); toast({ title: "Beneficiario creado" }); },
    onError: e => toast({ title: "Error", description: String(e), variant: "destructive" }),
  });
  const update = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => localApi(`/beneficiaries/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["beneficiaries"] }); setEditing(null); toast({ title: "Actualizado" }); },
    onError: e => toast({ title: "Error", description: String(e), variant: "destructive" }),
  });
  const del = useMutation({
    mutationFn: (id: string) => localApi(`/beneficiaries/${id}`, { method: "DELETE" }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["beneficiaries"] }); toast({ title: "Eliminado" }); },
    onError: e => toast({ title: "Error", description: String(e), variant: "destructive" }),
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Beneficiarios</h1><p className="text-sm text-muted-foreground">A quienes les realizas pagos</p></div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button size="sm"><Plus className="h-4 w-4 mr-1" /> Nuevo</Button></DialogTrigger>
          <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Nuevo beneficiario</DialogTitle></DialogHeader>
            <BenForm onSubmit={d => create.mutate(d)} onClose={() => setOpen(false)} />
          </DialogContent>
        </Dialog>
      </div>
      <Card><CardContent className="p-0">
        {list.isLoading ? <div className="p-6"><Skeleton className="h-32 w-full" /></div> : (
          <Table><TableHeader><TableRow>
            ...
            <TableBody>
              {(list.data?.length ?? 0) === 0 ? <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-8">Sin beneficiarios</TableCell></TableRow> :
              list.data?.map(b => (
                <TableRow key={b.id}>
                  <TableCell className="font-medium">{b.name}</TableCell>
                  <TableCell className="text-sm text-muted-foreground">{b.type}</TableCell>
                  <TableCell className="text-sm">{b.city ? `${b.city}, ` : ""}{b.country ?? "—"}</TableCell>
                  <TableCell className="text-sm">{b.phone1 ?? "—"}</TableCell>
      ...
      <Dialog open={!!editing} onOpenChange={o => !o && setEditing(null)}>
        <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Editar beneficiario</DialogTitle></DialogHeader>
          {editing && <BenForm initial={editing} onSubmit={d => update.mutate({ id: editing.id, data: d })} onClose={() => setEditing(null)} />}
        </DialogContent>
      </Dialog>
    </div>
  );
